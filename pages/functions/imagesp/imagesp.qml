...
<view class="cu-bar bg-white fixed">
    ...
    <view class="search-form round">
      ...
      <input type="text" placeholder="搜索 图片哈希" confirm-type="search" bindinput="onSearchInput"></input>
    </view>
  </view>
...
<view class="cu-list menu sm-border card-menu margin-top bg-white" qq:for="{{list}}" qq:key='index'  qq:if='{{item.show}}'>
 <view class="cu-item sm-border">
  <view class='content text-cut' style='max-width:70vw;'>
      <text class="text-blue">{{item.hash}}</text>
  </view>
  <button class='cu-btn round sm' data-item='{{item}}' catchtap='copyhash'>哈希</button>
  <button class='cu-btn round sm' data-item='{{item}}' data-index='{{index}}' catchtap='ViewImage' style='margin-left:10rpx;'>检查</button>
  <button class='cu-btn round sm bg-red' data-item='{{item}}' data-index='{{index}}' catchtap='opsome' data-op='not' style='margin-left:10rpx;'>屏蔽</button>
  <button class='cu-btn round sm' data-item='{{item}}' data-index='{{index}}' catchtap='opsome' data-op='yes' style='margin-left:10rpx;'>忽略</button>
 </view>
</view>
