...
<scroll-view scroll-y style="height: 100vh" bindscrolltolower="showmore">
  ...
  <view class="cu-list menu sm-border card-menu margin-top" qq:if="{{point_id && list.length>0}}">
    <view class="text-gray text-bold" style='text-align:center;'>Tips: 列表左滑可修改指定用户的银行固定资产</view>
  </view>
  <view class="justify-between" style='margin:20rpx;text-align:right;' qq:if="{{point_id && list.length>0}}">
  <button class="cu-btn bg-red sm radius" catchtap="deleteall">清空全员银行资产</button>
  </view>

  <view class="cu-list menu-avatar">
    <view class="cu-item {{modalName=='move-box-'+ index?'move-cur-half':''}}" qq:for="{{list}}" qq:key="item" style="border-radius: 20rpx; height: 160rpx" qq:if="{{item.show}}" data-item="{{item}}" bindtouchstart="ListTouchStart" bindtouchmove="ListTouchMove" bindtouchend="ListTouchEnd" data-target="move-box-{{index}}" qq:if='{{item.show}}'>
      ...
      <view class="move" style="max-width: 130rpx">
