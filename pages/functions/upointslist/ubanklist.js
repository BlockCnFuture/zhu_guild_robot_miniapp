...
Page({
  ...
  hideModal(e) {
    ...
    if (type == 'fine') {
      ...
      qq.request({
        url: `${app.globalData.host}/changeuserpointbank/${app.globalData.token}/${this.data.nowitem.point_id}/${this.data.nowitem.user_id}/${this.data.nowitem.points}`,
        success: (data) => {
          let ret = data.data;
          if (ret.errcode == 0) {
            this.setData({
  ...
  onShareAppMessage: function () {
    ...
    return {
      title: '初遇小竹 - 成员银行固定资产排行',
      path: `/pages/functions/upointslist/ubanklist?guildID=${app.globalData.userinfo.open_guild_id}`
    }
  },
  upchange(e) {
  ...
  deleteall: function () {
    ...
    qq.showModal({
      title: '警告',
      content: '此操作将清空所有成员的银行固定资产，您确认进行此操作吗？',
      success(res) {
        if (res.confirm) {
          sthis.setData({
            loadModal: true
          ...
          qq.request({
            url: `${app.globalData.host}/delallpointsbank/${app.globalData.token}/${sthis.data.point_id}`,
            success: (data) => {
              let ret = data.data;
              if (ret.errcode == 0) {
                sthis.setData({
  ...
  fetch_list: function () {
    ...
    qq.request({
      url: `${app.globalData.host}/upointslistbank/${app.globalData.token}/${this.data.point_id}/${tpage}`,
      success: (data) => {
        let ret = data.data;
        if (ret.errcode == 0) {
          if (ret.cnt > 0) {
