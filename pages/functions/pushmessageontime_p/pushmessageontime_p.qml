...
<view qq:if="{{nowtargetc=='nowtarget.cron'}}"></view>
<view qq:else>
...
<view class="cu-modal bottom-modal {{modalA=='show' && modalB=='' && modalC=='' && modalD==''?'show':''}}" bindtap="hideModalA">
  ...
  <view class="header-view">
      ...
      <text class="text-bold text-black text-lg margin-right-sm">查看定时推送</text>
      <view class="action text-black"> </view>
      </view>
  </view>
  </view>
  ...
  <view class="margin-sm margin-top">
    ...
    <view class="cu-timeline">
      ...
      <view class="cu-item cur cuIcon-form">
        <view class="content bg-blue light shadow-blur" style="text-align: left;">
          <view class="padding-top-xs">定时器设置<button class="cu-btn sm margin-left-sm shadow round" catchtap="setdesc" data-target='nowtarget.cron'>查看</button></view>
          <view style='width: 100%;word-wrap: break-word;'>
          <text style="text-align: left;">{{nowtarget.cron}}</text>
          </view>
        </view>
      </view>
      <view class="cu-item cur cuIcon-edit">
        <view class="content bg-blue light shadow-blur" style="text-align: left;">
          <view class="padding-top-xs">发送内容<button class="cu-btn sm margin-left-sm shadow round" catchtap="setdesc" data-target='nowtarget.content'>查看</button></view>
          <view style='width: 100%;word-wrap: break-word;'>
...
<view class="cu-bar bg-white fixed">
    ...
    <view class="search-form round">
      ...
      <input type="text" placeholder="搜索 昵称|时间|文本内容" confirm-type="search" bindinput="onSearchInput"></input>
    </view>
  </view>
...
<view class="bg-white" style="position:fixed;top:auto;left:0;right:0;z-index:999;">
  ...
  <button class="cu-btn bg-green sm" catchtap="opsome" data-op='0'>过审选中</button>
  <button class="cu-btn bg-red sm" catchtap="opsome" data-op='2'>拒审选中</button>
...
<view class="cu-list menu sm-border card-menu margin-top bg-white" qq:for="{{list}}" qq:key='index' bindtap='selectone' data-item='{{item}}' data-index='{{index}}' qq:if='{{item.show}}'>
 <view class="cu-item sm-border">
  <view class='content text-cut' style='max-width:70vw;'>
      ...
      <text class="text-blue text-cut">{{item.content}}</text>
  </view>
