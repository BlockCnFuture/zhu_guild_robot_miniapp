      ...
      <view class="cu-item sm-border">
        <view class="flex-sub text-center">
          <view class="solid-bottom text-xl padding">
            <text class="cuIcon-roundaddfill text-black text-bold"> 添加身份组奖励</text>
          </view>
        </view>
      </view>
    </view>
...
<view class="cu-list menu sm-border card-menu margin-top">
    <text>邀请码自动赋予积分设置</text>
</view>

<view qq:for="{{nowtarget.gpoints}}" qq:key="{{index}}">
    <view class="cu-list menu sm-border card-menu margin-top">
      <view class="cu-item sm-border arrow" bindtap='changepoint' data-item='{{item}}' data-index='{{index}}'>
        ...
        <text class="text-bold text-black">积分类型</text>
        </view>
        ...
        <view class="cu-tag round bg-green sm cuIcon-move margin-top-xs" catchtap="inputcnt" data-type="de" data-target='nowtarget.gpoints[{{index}}].pointcnt' data-cnt='{{item.pointcnt}}'></view>
        <view class="margin-lr text-black text-center solids-bottom" style="width:160rpx;">
      <input type="number" value="{{item.pointcnt}}" maxlength="10" placeholder-style="color:#000000" bindinput='inputcnts' data-target='nowtarget.gpoints[{{index}}].pointcnt' data-cnt='{{item.pointcnt}}' bindblur='finishinputcnt'></input>
    </view>
        <view class="cu-tag round bg-red sm cuIcon-add margin-top-xs" catchtap="inputcnt" data-type="add" data-target='nowtarget.gpoints[{{index}}].pointcnt' data-cnt='{{item.pointcnt}}'></view>
   ...
   <view class="cu-list menu sm-border card-menu margin-top" bindtap="addpoint">
      <view class="cu-item sm-border">
        <view class="flex-sub text-center">
          <view class="solid-bottom text-xl padding">
            <text class="cuIcon-roundaddfill text-black text-bold"> 添加积分奖励</text>
          </view>
        </view>
      </view>
    </view>
...
<view class="cu-bar bg-white fixed">
    ...
    <view class="search-form round">
      ...
      <input type="text" placeholder="搜索 邀请码" confirm-type="search" bindinput="onSearchInput"></input>
    </view>
  </view>
...
<view class="padding">
<button class="cu-btn block bg-blue margin-tb-sm lg" bindtap='addword'>创建新的邀请码</button>
</view>
</view>
