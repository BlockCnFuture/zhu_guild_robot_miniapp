...
Page({
  ...
  save: function () {
    ...
    qq.request({
      url: `${app.globalData.host}/invitecodeswr/${app.globalData.token}/${this.data.p}/1`,
      method: 'POST',
      data: t,
      success: (data) => {
        let ret = data.data;
  ...
  fetch_list: function () {
    ...
    qq.request({
      url: `${app.globalData.host}/fetchinvitecodes/${app.globalData.token}/${this.data.p}/${tpage}/1`,
      success: (data) => {
        let ret = data.data;
        if (ret.errcode == 0) {
          this.fetch_rolelist();
            ...
            if (this.data.keyword != '') {
              ...
              this.setData({
                list: ret.rows.map(item => {
                  return { ...item, show: item.code.toString().includes(s.toLowerCase()) };
                }),
                page: tpage + 1
              });
            } else {
  ...
  opsome: function () {
    ...
    qq.request({
      url: `${app.globalData.host}/delinvitecodes/${app.globalData.token}/${this.data.p}/1`,
      method: 'POST',
      data: { words: check },
      success: (data) => {
        let ret = data.data;
  ...
  addword: function () {
    ...
    if (check && check[1] && check[1].length > 4) {
      ...
      qq.showModal({
        title: '暂不支持',
        content: '您所在的频道暂不支持创建邀请码，请加入机器人官方频道进行反馈',
        showCancel: false
      })
    }
  },
  ...
  addrole: function () {
    if (this.data.nowtarget.groles.length >= 3) {
      qq.showModal({
        title: '达到上限',
        content: '最多可以设置三个身份组',
        showCancel: false
  ...
  kclick: function (event) {
    let points = this.data.nowtarget.gpoints;
    let item = event.currentTarget.dataset.item;
    points[this.data.nowindex].point_id = item.point_id
    points[this.data.nowindex].point_name = item.point_name;
    this.setData({
      'nowtarget.gpoints': points,
      modalC: ''
    })
  },
  addpoint: function () {
    if (this.data.nowtarget.gpoints.length >= 3) {
      qq.showModal({
        title: '达到上限',
        content: '最多可以设置三个积分奖励规则',
        showCancel: false
      });
      return;
    }
    this.data.nowtarget.gpoints.splice(this.data.nowtarget.gpoints.length, 1, {
      point_id: 0,
      point_name: '',
      pointcnt: 1
    })
    this.setData({
      'nowtarget.gpoints': this.data.nowtarget.gpoints
    });
  },
  ...
  delpoint: function (event) {
    let index = event.currentTarget.dataset.index;
    this.data.nowtarget.gpoints.splice(index, 1);
    this.setData({
      'nowtarget.gpoints': this.data.nowtarget.gpoints
    });
  }
