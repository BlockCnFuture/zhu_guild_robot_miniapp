  ...
  <view class="cu-dialog" catchtap="stopit">
    ...
    <scroll-view scroll-y>
      ...
      <view class="cu-bar bg-white flex padding justify-between">
        <button class="cu-btn bg-blue sm" catchtap="__selectall">全选</button>
        <button class="cu-btn bg-blue sm" catchtap="__cancleall">全不选</button>
      </view>
...
<view style="height:3vh;"></view>
<view class="text-gray" style="text-align: left;margin-left:30rpx;">发送内容支持以下变量，点击插入</view>
...
<view class="text-blue" style="text-align: left;margin-left:30rpx;font-size:34rpx;" bindtap='navigate_face' qq:if='{{nowtarget.markdown=="0"}}'><text class='cuIcon-emojifill text-yellow'></text> 小黄豆表情</view>
<view style='height:2vh;'></view>
<view class="text-gray" style="text-align: left;margin-left:30rpx;" qq:if='{{nowtarget.markdown=="1"}}'>支持以下markdown语法，点击插入，其他语法暂不支持</view>
<view class="text-blue" style="text-align: left;margin-left:30rpx;font-size:34rpx;" bindtap='addc' data-c='ma' qq:if='{{nowtarget.markdown=="1"}}'>- - - 分割线</view>
<view class="text-blue" style="text-align: left;margin-left:30rpx;font-size:34rpx;" bindtap='addc' data-c='mb' qq:if='{{nowtarget.markdown=="1"}}'>> 块引用</view>
<view class="text-blue" style="text-align: left;margin-left:30rpx;font-size:34rpx;" bindtap='addc' data-c='mc' qq:if='{{nowtarget.markdown=="1"}}'># 标题</view>
<view class="text-blue" style="text-align: left;margin-left:30rpx;font-size:34rpx;" bindtap='addc' data-c='md' qq:if='{{nowtarget.markdown=="1"}}'>- 无序列表</view>
<view class="text-blue" style="text-align: left;margin-left:30rpx;font-size:34rpx;" bindtap='addc' data-c='me' qq:if='{{nowtarget.markdown=="1"}}'>1. 有序列表</view>
<view style="height:3vh;"></view>
...
<view class="cu-modal bottom-modal {{modalA=='show' && modalB=='' && modalC=='' && modalD=='' && modal!='show'?'show':''}}" bindtap="hideModalA">
  <view class="cu-dialog" catchtap="stopit">
  <view class='header-view-top'>
  <view class="header-view">
      <view class="header">
      ...
      <text class="text-bold text-black text-lg margin-right-sm">{{nowtarget.id==''?'添加':'编辑'}}定时推送</text>
...
<view class="padding-right padding-top-sm">
<text class="text-bold text-black margin-right-sm">启用此推送任务</text>
      <switch class="sm" bindchange="RolesSwitch" data-target='nowtarget.useit' checked="{{nowtarget.useit=='1'}}"></switch>
</view>
  ...
  <view class="cu-list menu sm-border card-menu margin-top">
    <view class="cu-item sm-border">
      <text class="text-bold text-black">使用markdown格式推送</text>
      <switch class="sm" bindchange="RolesSwitch" data-target='nowtarget.markdown' checked="{{nowtarget.markdown=='1'}}"></switch>
    </view>
    <view class="cu-item sm-border">
      <text class="text-bold text-black">将图文分开推送</text>
      <switch class="sm" bindchange="RolesSwitch" data-target='nowtarget.imgtextdp' checked="{{nowtarget.imgtextdp=='1'}}"></switch>
    </view>
  </view>

  <view class="margin-sm margin-top">
    <view class="text-gray padding-bottom" style="text-align: left;">任务信息：</view>
...
<view class="cu-list menu sm-border margin-top">
      <view class="cu-item sm-border" data-index="0" bindtap="setcronru">
        <text class="text-bold text-black">定时_分钟</text>
      ...
      <view class="cu-item sm-border" data-index="1" bindtap="setcronru">
        <text class="text-bold text-black">定时_小时</text>
        <text class="text-gray" qq:if="{{cronanalyze[1]=='*'}}">每个小时</text>
        <view class="text-cut" style="max-width: 40vw" qq:else>
          <text class="text-gray">{{'每当第'+cronanalyze[1]+'小时'}}</text>
        </view>
      </view>
      <view class="cu-item sm-border" data-index="2" bindtap="setcronru">
        <text class="text-bold text-black">定时_日期</text>
        <text class="text-gray" qq:if="{{cronanalyze[2]=='*'}}">每一日</text>
        <view class="text-cut" style="max-width: 40vw" qq:else>
          <text class="text-gray">{{'每当第'+cronanalyze[2]+'日'}}</text>
        </view>
      </view>
      <view class="cu-item sm-border" data-index="3" bindtap="setcronru">
        <text class="text-bold text-black">定时_月份</text>
        <text class="text-gray" qq:if="{{cronanalyze[3]=='*'}}">每个月</text>
        <view class="text-cut" style="max-width: 40vw" qq:else>
          <text class="text-gray">{{'每当第'+cronanalyze[3]+'月'}}</text>
        </view>
      </view>
      <view class="cu-item sm-border" data-index="4" bindtap="setcronru">
        <text class="text-bold text-black">定时_周几</text>
      ...
      <view class="cu-item cur cuIcon-edit">
        <view class="content bg-blue light shadow-blur" style="text-align: left;">
          <view class="padding-top-xs">推送内容<button class="cu-btn sm margin-left-sm shadow round" catchtap="setdesc" data-target='nowtarget.content'>设置</button></view>
      ...
      <view class="cu-item cur cuIcon-list">
        <view class="content bg-blue light shadow-blur" style="text-align: left;">
         <view class="padding-top-xs">推送目标子频道<button class="cu-btn sm margin-left-sm shadow round" catchtap='setonchannels'>设置</button></view>
        <view class="cu-tag radius sm text-cut" style='justify-content:left;max-width:50vw;' qq:if='{{nowtarget.channel}}'>
<text qq:if='{{nowtarget.channel.type==0 && nowtarget.channel.sub_type==0}}' class="cuIcon-comment"><text>{{nowtarget.channel.name}}</text></text>
<text qq:if='{{nowtarget.channel.type==0 && nowtarget.channel.sub_type==1}}' class="cuIcon-notification"><text>{{nowtarget.channel.name}}</text></text>
<text qq:if='{{nowtarget.channel.type==2}}' class="cuIcon-voice"><text>{{nowtarget.channel.name}}</text></text>
<text qq:if='{{nowtarget.channel.type==10005}}' class="cuIcon-record"><text>{{nowtarget.channel.name}}</text></text>
<text qq:if='{{nowtarget.channel.type==10006}}' class="cuIcon-apps"><text>{{nowtarget.channel.name}}</text></text>
<text qq:if='{{nowtarget.channel.type==10007}}' class="cuIcon-edit"><text>{{nowtarget.channel.name}}</text></text>
        </view>
        </view>
      </view>
    </view>
...
<view class="cu-bar bg-white fixed">
    ...
    <view class="search-form round">
      ...
      <input type="text" placeholder="搜索 昵称|时间|发送内容" confirm-type="search" bindinput="onSearchInput"></input>
    </view>
  </view>
...
<view class="cu-list menu sm-border card-menu margin-top bg-white" qq:for="{{list}}" qq:key='index' bindtap='selectone' data-item='{{item}}' data-index='{{index}}' qq:if='{{item.show}}'>
 <view class="cu-item sm-border">
  <view class='content text-cut' style='max-width:70vw;'>
      ...
      <view class="text-blue text-cut" qq:if='{{item.useit=="1"}}'>{{item.content}}</view>
      <text class="text-gray" qq:else>未启用</text>
  </view>
  <button class='cu-btn round' data-item='{{item}}' data-index='{{index}}' catchtap='showinfo'>编辑</button>
    ...
    <view class="cu-item flex padding-left-sm margin-bottom-sm margin-top-sm">
      ...
      <view class="text-gray text-sm" style='text-align:right;'>
        ...
        <view class="text-gray" qq:if="{{item.task_type=='1'}}">审核中</view>
        <view class="text-blue" qq:if="{{item.task_type=='0'}}">审核通过</view>
        <view class="text-red" qq:if="{{item.task_type=='2'}}">审核不通过</view>
      </view>
    </view>
...
<view class="padding">
<button class="cu-btn block bg-blue margin-tb-sm lg" bindtap='addword'>增加定时消息推送</button>
</view>
</view>
