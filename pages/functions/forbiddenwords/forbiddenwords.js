...
Page({
  ...
  save: function () {
    ...
    if (t.keyword == '' || t.enabled_channels.trim() == '') {
      qq.showToast({
        title: '缺少必要内容',
        icon: 'none'
      })
      ...
      qq.request({
        url: `${app.globalData.host}/forbidenwordswr/${app.globalData.token}/${this.data.p}`,
        method: 'POST',
        data: t,
        success: (data) => {
          let ret = data.data;
  ...
  oclick: function (event) {
    ...
    if (item.type == '6' || item.type == '7' || item.type == '8' || item.type == '9') {
  ...
  fetch_list: function () {
    ...
    qq.request({
      url: `${app.globalData.host}/fetchforbidenkeywords/${app.globalData.token}/${this.data.p}/${tpage}`,
      success: (data) => {
        let ret = data.data;
        if (ret.errcode == 0) {
  ...
  opsome: function () {
    ...
    qq.request({
      url: `${app.globalData.host}/delforbidenwords/${app.globalData.token}/${this.data.p}`,
      method: 'POST',
      data: { words: check },
      success: (data) => {
        let ret = data.data;
